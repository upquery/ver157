...
begin 
    ...
    -- INICIO - select 
    ---------------------------------------------------- 
    SELECT SUM(LENGTH(JSON)), COUNT(*) into ws_count, ws_count2 FROM BI_ESTADOS;    
    ws_ds := 'BI_EST ('||ws_count||'-'||ws_count2||')';
    SELECT SUM(LENGTH(JSON)), COUNT(*) into ws_count, ws_count2 FROM BI_CIDADES;    
    ws_ds := ws_ds||', BI_CID ('||ws_count||'-'||ws_count2||')';
    SELECT SUM(LENGTH(JSON)), COUNT(*) into ws_count, ws_count2 FROM BI_ESTADOS_BRASIL;    
    ws_ds := ws_ds||', BI_EST_BRA ('||ws_count||'-'||ws_count2||')';
    SELECT SUM(LENGTH(JSON)), COUNT(*) into ws_count, ws_count2 FROM BI_CIDADES_BRASIL;    
    ws_ds := ws_ds||', BI_CID_BRA ('||ws_count||'-'||ws_count2||')';
    ------------------------------------------------
    -- FIM - select 
    ------------------------------------------------
    --

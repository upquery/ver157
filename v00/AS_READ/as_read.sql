...
--
  begin
    ...
    if dbms_lob.substr( p_xlsx, 4, 1 ) != hextoraw( '504B0304' ) then
      pipe row( t_one_cell );
      return;
    end if;
    ...
    for i in 1 .. t_sheet_ids.count loop
      ...
      then
        ...
        for r in 0 .. dbms_xmldom.getlength( t_nl3 )-1 loop
          ...
          if r = 0 then
              ws_tot := dbms_xmldom.getlength( t_nl2 );
          end if;
          
           ws_count := 0;
          
          --limite de celulas por linha
          ...
          for j in 0 .. dbms_xmldom.getlength( t_nl2 ) - 1 loop
            ...
            --ws_teste := dbms_xmldom.item( t_nl2, j );
            --if  ws_teste is not null then
            ...
            --while para preencher os vazios
            while col_alfan(rtrim( t_r, '0123456789' )) > t_one_cell.col_nr+1 loop
                ws_count := ws_count+1;
                t_one_cell.cell_type := 'S';
                t_one_cell.string_val := '';
                t_one_cell.row_nr := ltrim( t_r, rtrim( t_r, '0123456789' ) );
                t_one_cell.col_nr := t_one_cell.col_nr+1;
                t_one_cell.cell   := ws_count;
                pipe row( t_one_cell );
            ...
            -- t_one_cell.string_val := t_nl2;
            
            t_one_cell.row_nr := ltrim( t_r, rtrim( t_r, '0123456789' ) );
            t_one_cell.col_nr := col_alfan( rtrim( t_r, '0123456789' ) );
            t_one_cell.cell := t_r;
            ws_count := col_alfan( rtrim( t_r, '0123456789' ) );
            
            if p_cell is null or t_r = upper( p_cell ) then
                pipe row( t_one_cell );
            end if;
             --pipe row( t_one_cell );
          end loop;
            ...
            --while pós-colunas
            while ws_count < ws_tot loop
              ws_count := ws_count+1;
              t_one_cell.row_nr := ltrim( t_r, rtrim( t_r, '0123456789' ) );
                t_one_cell.cell_type := 'S';
                t_one_cell.string_val := '';         
               t_one_cell.col_nr := ws_count;
               t_one_cell.cell   := ws_count;
